...
Composite header chrome for the Journey Editor page. Replaces the inline header that lived inside `JourneyFlowEditor.tsx` with a clean 3-zone layout, ESC keyboard shortcut for Back, and a responsive kebab menu that collapses secondary actions below 1024px.
...
|---|---|---|---|
| `onBack` | `() => void` | — | Called by the Back button and the ESC shortcut. |
| `backLabel` | `string?` | `'Back'` | Pass already-translated text. |
| `title` | `string` | — | Identity (rendered as `<h1 className="text-lg font-semibold">`). |
| `subtitle` | `string?` | — | Secondary identity line (rendered as `<p className="text-sm text-muted-foreground">`). Omitted entirely when absent. |
...
| `lastSaved` | `Date \| null?` | `null` | When set, renders a Clock icon + formatted time in the actions zone (hidden below `md`). |
...
[ data-zone="navigation" ]  ·  [ data-zone="identity" (flex-1) ]  ·  [ data-zone="actions" ]
     Back button                  Title + subtitle                  last-saved · Visualizar | Env | Save
                                                                              (kebab below 1024px)
```
...
| Key | Action | When it does NOT fire |
|---|---|---|
| `Escape` | Calls `onBack()`. | When focus is in `<input>` / `<textarea>` / `<select>` / `[contenteditable]`. Also when any Radix Dialog is open (Radix catches Escape with higher priority via its own internal listener). |
...
| Viewport | Layout |
|---|---|
| `≥1024px` (`lg`) | Back · Identity · (last-saved · View sessions · Env · Save) — full horizontal layout. |
| `<1024px` | Back · Identity · (Env · Kebab · Save) — View sessions moves into the kebab `DropdownMenu`. |
| `<768px` (`md`) | Same as above, plus the last-saved timestamp is hidden (`hidden md:flex`). |
...
- Back button declares `aria-keyshortcuts="Escape"`.
- All icons are `aria-hidden="true"`.
- Loading state on Save uses `aria-live="polite"` via the surrounding container (last-saved indicator is also `aria-live="polite"` so timestamp updates are announced).
...
- **Storybook:** `pnpm storybook` → "Flow Builder / JourneyEditorHeader" (6 stories: Pristine / Dirty / Saving / WithLastSaved / WithoutSubtitle / PortugueseLabels).
- **Tests:** `pnpm test src/components/journey/shared/JourneyEditorHeader` — 18 cases covering layout zones, all 4 ACs, and edge cases (ESC in INPUT/TEXTAREA/contentEditable, listener cleanup, kebab menu item dispatch).
