...
| `serverSnapshot` | `FlowSnapshot \| null` | The data shape we believe the server holds. Updated only on `commitSave`. |
...
| `commitSave(savedAt)` | Server returned success. `savedAt` should be `new Date()` (wall-clock of save completion). |
...
| File | Tests |
|---|---|
| `useFlowEditorStore.spec.ts` | 20 — hydration, recovery candidate selection, dirty propagation, debounce reset on edits, save lifecycle (begin/commit/fail), error → dirty on new edit, 30s auto-retry, retry cancellation paths (commit / manual save / new edit / consecutive failure restart). |
